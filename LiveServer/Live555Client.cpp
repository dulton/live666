...
int Live555Client::LiveTrack::init()
{
    ...
    if( !strcmp( sub->mediumName(), "audio" ) )
    {
        ...
        else if( !strcmp( sub->codecName(), "VORBIS" ) )
        {
                                                ...
                                                i_extra ) ) )
            {
                ...
                onDebug("Missing or unsupported vorbis header." );
        }
    }
    else if( !strcmp( sub->mediumName(), "video" ) )
    {
        fmt.i_codec = VLC_CODEC_UNDEF;
        fmt.i_cat = VIDEO_ES;
    ...
    else if( !strcmp( sub->mediumName(), "text" ) )
    {
       ...
       {
           fmt.i_codec = VLC_CODEC_ITU_T140;
       }
    }

    /* Try and parse a=lang: attribute */
    char* p_lang = strstr( sub->savedSDPLines(), "a=lang:" );
    if( p_lang )
    {
       unsigned i_lang_len;
       p_lang += 7;
       i_lang_len = strcspn( p_lang, " \r\n" );
       fmt.text.psz_language = strndup( p_lang, i_lang_len );
    }
    else{
        p_lang = p_sess_lang;
    }
